...
remote_versions[oc]="curl -sL https://mirror.openshift.com/pub/openshift-v4/clients/ocp/latest/release.txt | awk '/Version:/ { print \$NF }'"
remote_versions[helm]="curl -sL https://github.com/helm/helm/releases/latest | sed -E -n 's/.*tag\/(v[0-9\.]+).*/\1/p' | tail -n1"
remote_versions[krew]="curl -sL https://github.com/kubernetes-sigs/krew/releases/latest | grep -m1 'kubernetes-sigs/krew/releases/tag' | sed -E -n 's/.*tag\/(v[0-9\.]+).*/\1/p'"
...
remote_versions[knative]="curl -sL https://github.com/knative/client/releases/latest | grep -m1 'knative/client/releases/tag' | sed -E -n 's/.*knative-(v[0-9\.]+).*/\1/p'"
remote_versions[argocd]="curl -sL https://github.com/argoproj/argo-cd/releases/latest | grep -m1 'argoproj/argo-cd/releases/tag' | sed -E -n 's/.*tag\/(v[0-9\.]+).*/\1/p'"
remote_versions[roxctl]="curl -sL  https://mirror.openshift.com/pub/rhacs/assets/latest/bin/Linux/sha256sum.txt | cut -d' ' -f1"

# declare the method to get the current version of the application
declare -A local_versions
local_versions[oc]="oc version | awk '/Client Version/ { print \$NF }'"
local_versions[helm]="helm version | sed -E -n 's/.*Version:.(v[0-9\.]+)..*/\1/p'"
local_versions[krew]="kubectl-krew version | awk '/GitTag/ { print \$NF }'"
local_versions[tekton]="tkn version | awk '/Client version/ { print \$NF }'"
